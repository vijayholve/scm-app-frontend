...
// package may not be available, so we conditionally require it at runtime.
let LanguageDetector = null;
try {
  // Use require so bundlers that don't have the package won't fail at parse time
  // when running in native environments.
  // eslint-disable-next-line global-require
  LanguageDetector = require('i18next-browser-languagedetector');
  // Some environments export the detector as default
  if (LanguageDetector && LanguageDetector.default) LanguageDetector = LanguageDetector.default;
} catch (e) {
