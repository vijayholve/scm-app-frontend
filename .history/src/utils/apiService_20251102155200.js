...
export const getUserSchoolClassDivision = async () => {
  ...
  if (user?.type === 'STUDENT') {
    return { schoolId: user.schoolId || null, classId: user.classId || null, divisionId: user.divisionId || null };
  }
...
export const getDocumentsByAccountAndUser = (accountId, userId) => apiClient.get(`/api/documents/${accountId}/${userId}`);

export const downloadUserDocument = (accountId, userId, documentId) => apiClient.get(`/api/documents/download/${accountId}/${userId}/${documentId}`, { responseType: 'blob' });
...
export const checkMultipleCourseEnrollments = async (accountId, courseIds, studentId) => {
  try {
    ...
    return courseIds
      .map((courseId, index) => ({ courseId, enrollmentStatus: results[index] }))
