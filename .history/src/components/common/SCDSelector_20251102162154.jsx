...
const getMockData = () => ({
  schools: [
    ...
    { id: 2, name: 'Central Secondary School' },
  ],
  classes: [
    { id: 101, name: 'Grade 9', schoolId: 1 },
    { id: 102, name: 'Grade 10', schoolId: 1 },
    { id: 103, name: 'Grade 11', schoolId: 2 },
  ],
  divisions: [
    { id: 201, name: 'A Section', schoolId: 1 },
    { id: 202, name: 'B Section', schoolId: 1 },
    { id: 203, name: 'C Section', schoolId: 2 },
  ],
});


// Reimplementing useSCDData to simulate an asynchronous fetch from an API
const useSCDData = () => {
  ...
  useEffect(() => {
    ...
    // Function to simulate fetching data from an API endpoint
    const fetchData = async () => {
      setLoading(true);
      try {
        // Simulating a network call delay of 1 second (e.g., fetch('/api/scd-data'))
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        const data = getMockData(); // Retrieve "API" data
  ...
  return { schools, classes, divisions, loading };
};

const userDetails = {
  // Mocking an ADMIN and a TEACHER
  getUser: () => ({
    // Change this type to test TEACHER vs ADMIN behavior
    type: 'TEACHER', // 'ADMIN' or 'TEACHER'
    schoolId: 1, // Only relevant for TEACHER
    allocatedClasses: [
      { classId: 101, divisionId: 201 },
      { classId: 102, divisionId: 202 }
    ],
  }),
};
// ------------------------------------
...
/**
 * Reusable component to replace Material UI Autocomplete/TextField pattern.
 * Uses a fixed-position div (Modal replacement) and mapped list (FlatList replacement) for selection.
 */
const DropdownSelect = ({
  ...
  // Merge base style with error/disabled styles
  const inputStyle = {
    ...styles.input,
  ...
  return (
    <div style={styles.inputContainer}>
      ...
      <button
        style={{ ...inputStyle, ...styles.touchableArea }} // Use button for TouchableOpacity replacement
        onClick={() => !disabled && setModalVisible(true)}
        disabled={disabled}
      >
        <span style={styles.valueText}>{selectedLabel || `Select ${label}...`}</span>
      </button>

      {errorText && <span style={styles.errorText}>{errorText}</span>}

      {/* Modal Replacement: Fixed position div */}
      {modalVisible && (
        <div style={styles.modalOverlay}>
          <div style={styles.modalContent}>
            <span style={styles.modalTitle}>{`Select ${label}`}</span>
            <div style={styles.flatListReplacement}>
              {(options || []).map((item, index) => (
                <button
                  key={item.id}
                  style={{ ...styles.optionItem, ...(index > 0 ? styles.separator : {}) }}
                  onClick={() => handleSelect(item)}
                >
                  <span style={styles.optionText}>{getOptionLabel(item)}</span>
                </button>
              ))}
            </div>
            <button
            ...
            >
              <span style={styles.modalCloseButtonText}>Close</span>
            </button>
          </div>
        </div>
      )}
