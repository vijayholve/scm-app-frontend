...
import { Card, Text, Button, ActivityIndicator, IconButton, Portal, Dialog } from 'react-native-paper';
...
const ReusableDataGrid: React.FC<DataGridProps> = ({
  ...
  const fetchData = useCallback(async (pageToFetch: number, isRefreshing = false) => {
  ...
  const handleEdit = (item: any) => {
    if (editRoute) {
      ...
      navigation.navigate(editRoute as never, { id: item._id, data: item } as never);
    } else {
      Alert.alert('Configuration Error', 'Edit route not configured for this data grid.');
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || `Failed to delete ${title.slice(0, -1)}.`;
            ...
            <View style={styles.valueContainer}>
              {column.isAction ? (
                ...
                column.renderCell ? (
                  ...
                  <Text>{item[column.key] ? String(item[column.key]) : 'N/A'}</Text>
                )
              )}
            </View>
          </View>
  ...
  const renderFooter = () => {
    ...
    if (refreshing) return <ActivityIndicator style={styles.loader} size="small" />;
    ...
    if (page >= totalPages && data.length > 0) {
        return <Text style={styles.endOfListText}>--- End of {title} List ---</Text>;
    }
    ...
    <View style={styles.container}>
      ...
      {data.length === 0 && !loading && !error ? (
        <View style={styles.emptyContainer}>
            ...
            <Button mode="outlined" onPress={handleRefresh}>Reload</Button>
        </View>
      ) : (
        <FlatList
          data={data}
          keyExtractor={(item) => item._id || item.id || Math.random().toString()} // Ensure unique key
          ...
          refreshControl={
            <RefreshControl refreshing={refreshing && page === 1} onRefresh={handleRefresh} />
          }
        />
      )}
      ...
      <Portal>
        <Dialog visible={showDeleteDialog} onDismiss={() => setShowDeleteDialog(false)}>
          ...
          <Dialog.Actions>
            <Button onPress={() => setShowDeleteDialog(false)} disabled={loading}>Cancel</Button>
